...
function isCreditCardNumber(ccn) {
  ...
  const sum = array.reduce((acc, n, index) => {
    const num = Number(n);
    if (index % 2) {
      if (num < 5) return acc + (num * 2);
      return acc + Number(`${num * 2}`[0]) + Number(`${num * 2}`[1]);
    }
    return acc + num;
  }, 0);
  return !(sum % 10);
...
function isBracketsBalanced(string) {
  ...
  while (/\(\)/.test(str) || /\[\]/.test(str) || /\{\}/.test(str)
    || /\|\|/.test(str) || /<>/.test(str));
...
function getCommonDirectoryPath(pathes) {
  ...
  arr[0].forEach((folder, i) => {
    if (arr.every((path) => folder === path[i])) {
      answer += `${folder}/`;
    }
  });
  return answer;
